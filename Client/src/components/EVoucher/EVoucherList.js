...
import { Grid, InputAdornment, Table, TableBody, TableCell, TableRow,   TableContainer,Paper } from '@material-ui/core'
...
const EVoucherList = (props) => {
    ...
    const headCells = [
        ...
        { id: 'AccessPhoneNumber', label: 'Phone', },
        { id: 'Used', label: 'Condition', }
    ]
    ...
    useEffect(() => {
        setLoading(true)
        const getEvoucher = async () => {
            const res = await getData('EVoucher/GetAllEVoucherList')
            if (res.ok) {
                setEVoucher(res.data.reverse())
            }
            setLoading(false)
        }
        getEvoucher();
    ...
    const { TblHead, TblPagination, recordAfterPagingAndSorting } = UseTable(Evoucher, headCells, filterFn);
                                                ...
                                                Evoucher.length <= 0 ? (<>
                                                    ...
                                                    recordAfterPagingAndSorting().map((item, index) => (
                                                        <TableRow key={index} classes={{ root: index === 0 || index % 2 === 0 ? classes.tableRowBy2 : classes.tableRow }} >
                                                            ...
                                                            <TableCell className={classes.tableCell}>{convertUTCDateToLocalDate(item.expireDate)}</TableCell>
                                                            <TableCell className={classes.tableCell}>{item.promocode}</TableCell>
                                                            <TableCell className={classes.tableCell}>{item.accessPhoneNumber == null ? "own": item.accessPhoneNumber}</TableCell>
                                                            <TableCell className={classes.tableCell}>{item.used ? "used" : "unused"}</TableCell>
